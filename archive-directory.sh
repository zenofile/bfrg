...
readonly \
    ...
    DATA_REDUNDANCY=5 \
    VERBOSE=1 \
    SAFE_TMP='/home/zeno/cstorage/cryptmp/' \
    SOURCE_PATH='/home/zeno/cstorage/secrets' \
    BLK_LOCAL_TARGETS=('/home/zeno/cstorage/backup/secrets' '/home/zeno/storage/backup/secrets' '/home/zeno/storage/sync/backup/secrets' '/run/media/zeno/SD_SECRETS/secrets' '/run/media/zeno/FLASH_SECRET') \
    SCP_REMOTE_TARGETS=() \
    RSYNC_REMOTE_TARGETS=('zeno@sirius.fritz.box:/volume1/homes/zeno/rsync_backup/secrets') \
...
_check_tools() {
    ...
    if ! _require_command rclone; then
        if (( ${#RCLONE_REMOTE_TARGETS[@]} > 0)); then
            _get_user_input "rclone remote paths exist but no rclone binary found, continue anyway?"
        fi
